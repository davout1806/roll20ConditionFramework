...
Modified by me in order to pass along selected tokens
...
community.command.commandList["?"].handle = function(args, who, selected, isGM) {
    log("HHH " + selected);
    if(args.length == 1) {
...
// Parse the command
community.command.parse = function(message, who, selected) {
    log("SEL "+ selected);
    ...
    if(command in community.command.commandList) {
        ...
        if("handle" in community.command.commandList[command]) {
            ...
            if(arr.length >= minArgs && (arr.length <= maxArgs || maxArgs == 0)) {
                ...
                // OMG an error!!!
                if(error) return;
                log("LLL " + JSON.stringify(community.command.commandList[command]));

                community.command.commandList[command].handle(args, who, selected, isGM);
...
on("chat:message", function(msg) {
    if(msg.type == "api" && msg.content.indexOf("!") !== -1) {
        community.command.parse(msg.content, msg.who, msg.selected);
    }
});